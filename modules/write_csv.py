...
def save_fit(FED, ch, ietamin, ietamax, thisside, slope,unc_slope, intercept, unc_intercept, chi2, TCDS, suffix = "test"):
    #if FED == -1:
    ...
    if FED:
        ...
        if thisside >-1: chorieta = "side"
        if ietamax > -999 and ietamin > -999: ch = statistics.mean([ietamax,ietamin])
        if thisside > -1: ch = 1
        print('fits_results/temperature/FEDs_'+chorieta+'.csv')
        if os.path.isfile('fits_results/temperature/FED'+str(FED)+'_'+chorieta+'_'+suffix+'.csv'):
            df = pd.read_csv('fits_results/temperature/FED'+str(FED)+'_'+chorieta+'_'+suffix+'.csv', index_col=[0,1],  header=[0, 1], skipinitialspace=True)
            ...
            df.to_csv('fits_results/temperature/FED'+str(FED)+'_'+chorieta+'_'+suffix+'.csv', mode='w')
        else:
            ...
            df.to_csv('fits_results/temperature/FED'+str(FED)+'_'+chorieta+'_'+suffix+'.csv')
    #else:
